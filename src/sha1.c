...
static uint32_t
sha1_transform(SHA1 *sha1, const uint8_t buffer[SHA1_BLOCK_LENGTH])
{
	uint32_t a, b, c, d, e;
...
void
sha1_update(SHA1 *sha1, const uint8_t *data, size_t length)
{
	size_t i, j;
...
void
sha1_final(SHA1 *sha1, uint8_t digest[SHA1_DIGEST_LENGTH])
{
	uint32_t i;
	uint8_t count[8];
